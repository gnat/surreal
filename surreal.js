...
let $ = {//Convenience for internals.
	...
	classRemove(e, name) {//Remove class from element(s).
		if (typeof name !== 'string') return e
		if (name.charAt(0) === '.') name = name.substring(1)
	...
	classToggle(e, name, force) {//Toggle class in element(s).
		if (typeof name !== 'string') return e
		if (name.charAt(0) === '.') name = name.substring(1)
	...
	enable(e) {//For reversing disable()
		if ($.isNodeList(e)) e.forEach(_=>{$.enable(_)})
		if ($.isNode(e)) e.disabled = false
		return e
	},
	send(e, name, detail=null) {//Send / trigger event. ✂️ Vanilla: Events Dispatch: document.querySelector(".thing").dispatchEvent(new Event('click'))
	...
	halt(ev, keepBubbling=false, keepDefault=false) {//Halt event. Default: Stops normal event actions and event propagation.
	...
	attribute(e, name, value=undefined) {//Add or remove attributes from element(s)
	...
	globalsAdd() {//Puts Surreal functions except for "restricted" in global scope.
	...
	isNode(e) {//⚙️ Used internally. Is this an element / node?
		return (e instanceof HTMLElement || e instanceof SVGElement || e instanceof Document) ? true : false
	},
	isNodeList(e) {//⚙️ Used internally by DOM functions. Is this a list of elements / nodes?
		return (e instanceof NodeList || Array.isArray(e)) ? true : false
	},
	isSelector(selector="", start=document, warning=true) {//⚙️ Used internally by DOM functions. Warning when selector is invalid. Likely missing a "#" or "."
...
function pluginEffects(e) {
	function fadeOut(e, f=undefined, ms=1000, remove=true) {//Fade out and remove element. Equivalent to jQuery fadeOut(), but actually removes the element!
		let thing = e
	...
	function fadeIn(e, f=undefined, ms=1000) {//Fade in an element that has opacity under 1
		let thing = e
...
//Example: <script>onloadAdd(_=>{console.log("Page was loaded!")})</script>
//Example: <script>onloadAdd(_=>{console.log("Lets do another thing without clobbering window.onload!")})</script>
